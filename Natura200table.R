...
All <- c(Rast_Natura2000_Croped, Rast_Habitatnaturtype_terrestrial_Croped, Rast_p3_klit_Croped, Rast_NaturaOgVildtreservater_Croped, Rast_IUCN_Croped, Rast_Urort_Skov_Croped, Rast_National_Parks_Croped, Rast_stoette_Croped, Rast_Fondsejede_Croped, Forerst, Rast_markblokkort_Croped, Rast_Ownersip_Croped)
...
Table2_Total <- Long_Table_All2 %>% dplyr::filter(!is.na(Natura_2000) | !is.na(Habitatnaturtype) | !is.na(NaturaOgVildtreservater) | !is.na(IUCN) | !is.na(Urort_Skov) | !is.na(Naturnationalparks) | !is.na(Stoette) | !is.na(Fond) | !is.na(Habitats_P3)) %>%
...
Table2_All_Total <- list(Table2_Total, Table2_Open_Nature, Table2_Skovnatur, Table2_Soer, Table2_Drevet_Skov, Table2_PGR2, Table2_PGR,  Table2_OMD) %>%
...
Table2_Drevet_Skov <- Long_Table_All2 %>%
  dplyr::filter(!is.na(Naturnationalparks)) %>%
  dplyr::filter(is.na(Forest) & is.na(Naturnationalparks)) %>%
  dplyr::filter(is.na(Habitats_P3), is.na(Urort_Skov), !is.na(Forest), is.na(Types_markblokkort)) %>%
  summarise(Area_Sq_Km = sum(Area_Sq_Km)) %>%
  rename(Drevet_Skov = Area_Sq_Km) %>%
  mutate(type = "Naturnationalparks")
...
Long_Table_All2 <- read_rds("NewTable2.rds")
